...
int main()
{
  ...
  for (int z=0;z<NVARIABLES;++z) {
	...
	std::cout << "Done " << histoName << std::endl;
      ...
      myPlot->setMCHist(DiphotonPlot::iQCD,   histos[1][j][z]);
      myPlot->setMCHist(DiphotonPlot::iGJets, histos[2][j][z]);
      myPlot->setMCHist(DiphotonPlot::iGG,    histos[3][j][z]);
      myPlot->setMCHist(DiphotonPlot::iRS,    histos[0][j][z]);
